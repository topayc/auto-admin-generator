	...
	A3Maker.preview = {
		...
		initializeCloseEventHandler: function () {
			...
			$('.preClose').click(function () {
				...
				if (A3Maker.newPageWindow.isShown) {
					A3Maker.newPageWindow.showTemporary();
				}
			});
		},
		...
		startPreview: function () {
			...
			this.previewWidth = A3Maker.project.width;
			this.previewHeight = A3Maker.project.height;
			this.$previewContainer = $(A3Maker.previewWindowHTML);
			...
			this.$previewContainer.fadeIn(120);
			this.setFullScreen();
		...
		setFullScreen: function (id) {
			//var target = document.querySelector('#preBg')
			//A3Maker.util.fullScreen(target);
			var target = $('#preBg')[0];
		...
		callExternalPreview: function (memberSeq, projectSeq) {
			...
			var context = this;
			A3Maker.api.A3MakerApi('loadTemplate', { member_seq: memberSeq, project_seq: projectSeq }, function (result) {
		...
		startPreviewFromJsonObject: function (project) {
			...
			if (A3Maker.newPageWindow) {
				if (A3Maker.newPageWindow.isShown) A3Maker.newPageWindow.hideTemporary();
			}
		...
		createDomAndAppend: function ($container, component, componentType) {
			if (componentType == 'image') {
				...
				$img.addClass("component").addClass("component-image").data('component', 'image')
					.attr('src', A3Maker.resource.getImageResourcePathByBasePath(this.header.base_path, component.src, 'image'));
				...
				if ($img.attr('src') == '' ||
					...
					$img.attr('src') == 'none' ||
					$img.attr('src') == A3Maker.resource.getDefaultResourcePath('image')) {
					$img.css('opacity', 0);
				}
				return $img;
			}
			...
			if (componentType == 'order') {
				...
				$order.addClass("component").addClass("component-order").data('component', 'order')
					.attr('src', A3Maker.resource.getImageResourcePathByBasePath(this.header.base_path, component.src, 'order'));
				...
				$order.attr('item_name', component.menuName);
				$order.attr('item_price', component.menuPrice);
				$order.attr('item_code',  component.menuCode);
				$order.attr('store_code', A3Maker.dummy.storeCode);
				...
				if ($order.attr('src') == '' ||
					...
					$order.attr('src') == 'none' ||
					$order.attr('src') == A3Maker.resource.getDefaultResourcePath('order')) {
					$order.css('opacity', 0);
				}
				return $order;
			}
			...
			if (componentType == 'link') {
				if (component.linktype == "YouTube") {
					...
					$youTubeElem.attr('src', A3Maker.resource.createYoutubeLInk(component.target));
					$container.append($youTubeElem);
					return $youTubeElem;
				} else {
					var $link = $("<img/>");
					$link.addClass("component").addClass("component-link").data('component', 'link')
						.attr('src', A3Maker.resource.getImageResourcePathByBasePath(this.header.base_path, component.src, 'link'));
					...
					if ($link.attr('src') == '' ||
						...
						$link.attr('src') == 'none' ||
						$link.attr('src') == A3Maker.resource.getDefaultResourcePath('link')) {
						$link.css('opacity', 0);
					}
					return $link;
				}
		...
		initCloneDom: function () {
			var context = this;
			$('.preview_content').find(".cross_grid_line").remove();
			$('.preview_content .page-border').removeClass("page-border");
			//$('.preview_content').find(".moveActionTrigger").removeClass();
			//$('.preview_content').find(".internalWrapper").removeClass();
			//$('.preview_content').find(".component").removeClass();
			$('.preview_content').find(".grip").remove();
			$('.preview_content').find(".component_border").remove();
			$('.preview_content').find("*").css('cursor', 'default');
			$('.preview_content').find(".align_component_line").remove();
			...
			/*
			$('.preview_content .editor-page').css({'overflow':'hidden', 'position' : 'absolute' , 'cursor' : 'default'});
			$('.preview_content .editor-page').hide();  // first hide all page 
			$('.preview_content .editor-page').removeClass('cross_grid_line');   // remove grid line
			$('.preview_content .component_border').remove(); //remove gray panels for popup  in editing mode
			$('.preview_content .component').css({'cursor' : 'default'});
			...
			*/
			...
			$('.preview_content .component-link').each(function (index) {
				...
				if (linktype == 'YouTube') {
					...
					$youTubeElem.attr('src', A3Maker.resource.createYoutubeLInk(position));
					$(this).replaceWith($youTubeElem);
				} else {
					if ($(this).attr('src') == '' ||
						$(this).attr('src') == 'undefined' ||
						$(this).attr('src') == 'none' ||
						$(this).attr('src') == A3Maker.resource.getDefaultResourcePath('link')) {
						$(this).css('opacity', 0);
						$(this).addClass("component").addClass("component-link").data('component', 'link')
					}
				}
			});
			...
			$('.preview_content .component-image').each(function (event) {
				if ($(this).attr('src') == '' || $(this).attr('src') == 'undefined' ||
					$(this).attr('src') == 'none' || $(this).attr('src') == A3Maker.resource.getDefaultResourcePath('image')) {
					$(this).css('opacity', 0);
				}
			});
			...
			$('.preview_content .component-order').each(function (index) {
				if ($(this).attr('src') == '' || $(this).attr('src') == 'undefined' ||
					$(this).attr('src') == 'none' || $(this).attr('src') == A3Maker.resource.getDefaultResourcePath('order')) {
					$(this).css('opacity', 0);
				}
			});
		...
		handleOrder: function (eventSource) {
			var itemName = $(eventSource).attr('item_name');
			var itemPrice = $(eventSource).attr('item_price');
			var itemCode = $(eventSource).attr('item_code');
			var storeCode = $(eventSource).attr('store_code');

			alert(`주문 전송 ( 가맹점 코드 : ${storeCode} , 상품코드 : ${itemCode},  상품 이름 : ${itemName}, 상품 가격 : ${itemPrice} )`);
		},
		...
		close: function () {
			...
			this.$preview.empty();
			this.curPageInx = -1;
			this.$previewContainer.fadeOut(120);
